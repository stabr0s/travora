alter table public.trips
  add column if not exists public_share_sections jsonb not null default '{"overview":true,"places":true,"planner":true,"reservations":true,"budget":true,"packing":true}'::jsonb;
do $$
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'trips_public_share_sections_object_check'
      and conrelid = 'public.trips'::regclass
  ) then
    alter table public.trips
      add constraint trips_public_share_sections_object_check
      check (jsonb_typeof(public_share_sections) = 'object');
  end if;
end;
$$;
...
as $$
  with shared_trip as (
    select
      ...
      jsonb_build_object(
        'overview', true,
        'places', case
          when jsonb_typeof(public_share_sections -> 'places') = 'boolean' then (public_share_sections ->> 'places')::boolean
          else true
        end,
        'planner', case
          when jsonb_typeof(public_share_sections -> 'planner') = 'boolean' then (public_share_sections ->> 'planner')::boolean
          else true
        end,
        'reservations', case
          when jsonb_typeof(public_share_sections -> 'reservations') = 'boolean' then (public_share_sections ->> 'reservations')::boolean
          else true
        end,
        'budget', case
          when jsonb_typeof(public_share_sections -> 'budget') = 'boolean' then (public_share_sections ->> 'budget')::boolean
          else true
        end,
        'packing', case
          when jsonb_typeof(public_share_sections -> 'packing') = 'boolean' then (public_share_sections ->> 'packing')::boolean
          else true
        end
      ) as sections
    from public.trips
  ...
  place_rows as (
    ...
    where (t.sections ->> 'places')::boolean
  ),
  planner_rows as (
    select coalesce(
      jsonb_agg(
    ...
    where (t.sections ->> 'planner')::boolean
  ),
  reservation_rows as (
    select coalesce(
      jsonb_agg(
    ...
    where (t.sections ->> 'reservations')::boolean
  ),
  budget_rows as (
    select coalesce(
      jsonb_agg(
    ...
    where (t.sections ->> 'budget')::boolean
  ),
  packing_rows as (
    select coalesce(
      jsonb_agg(
    ...
    where (t.sections ->> 'packing')::boolean
  )
  select case
    when not exists (select 1 from shared_trip) then null
    else jsonb_build_object(
      'sections', (select sections from shared_trip),
      'trip', (
